...
export const createElementClass = <P>(
  ...
  return class extends HTMLElement {
    ...
    private _safeReadAttribute<A extends keyof P & string>(attrName: A): Optional<P[A]> {
      ...
      if (type === 'string') {
        return (attrValue ?? undefined) as unknown as Optional<P[A]>;
      }
      if (type === 'number') {
        return (attrValue ? Number(attrValue) : undefined) as unknown as Optional<P[A]>;
      }
      if (type === 'boolean') {
        return (attrValue ? Boolean(attrValue) : undefined) as unknown as Optional<P[A]>;
      }
      if (type === 'object') {
